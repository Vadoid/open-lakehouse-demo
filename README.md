...
- **Option 1 (default)** is the full Spark + Flink stack: it adds two containers
  ...
  confirms the cluster is up and idle, and prints interop instructions. The streaming job
...
- Pin versions: `lakekeeper_version` (default `v0.12.0`) in `variables.tf`, and
  the Spark Docker image tag and `ICEBERG_VER` in `main.tf`. The Lakekeeper
